...
async def storge_data(from_wxid, text_prompt, response, local_time, model_name):
    if not os.path.exists(file_name):
        ...
        user_df = pd.DataFrame(
            [[from_wxid, text_prompt, response, local_time, model_name]],
            columns=["from_wxid", "question", "response", "date_time", "model_name"],
            index=[len(user_QA)],
        )
